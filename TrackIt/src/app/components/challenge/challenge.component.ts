...
export class ChallengeComponent implements OnInit {
  ...
  addedFriends: { id: string; name: string; imageUrl: string; activeChallenge: string }[] = [];
      ...
      if (friend) {
        ...
        if (!alreadyAdded) {
          this.addedFriends.push({
            id: friend.id, // fontos: ez legyen a Friend_Request ID-ja
            name: friendName,
            imageUrl: friendImage,
            activeChallenge: this.user_challenges?.id || ''
          });
        }
      }
    });
  ...
  onFriendSubmit() {
    ...
    const missingChallenges = this.addedFriends.filter(friend => !friend.activeChallenge);
    if (missingChallenges.length > 0) {
      this.invalidFields = missingChallenges.map(friend => 'activeChallenge');
      this.modalVisible = true;
      this.modalType = 'error';
      this.modalMessage = 'Hiányzik a kihívás megadása néhány baráthoz!';
      return;
    }
    
    const updateRequests = this.addedFriends.map(friend => {
      return this.api.updateFriendData(friend.id, {
        activeChallenge: friend.activeChallenge
      }).toPromise();
    });
    ...
    Promise.all(updateRequests)
      .then(() => {
        ...
        this.modalMessage = 'Minden barát frissítve lett!';
        this.invalidFields = [];
        this.autoCloseModal();
    
        this.addedFriends.forEach(friend => {
          friend.activeChallenge = friend.activeChallenge;
        });
      })
      .catch(error => {
        ...
        this.modalVisible = true;
        this.modalType = 'error';
        this.modalMessage = error.error?.message || 'Hiba történt a barátok frissítése során.';
